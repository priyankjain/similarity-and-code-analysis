...
def updateProjectDict(projectDict, projectName, percentage, linesMatched):
	if projectName in projectDict:
		...
		projectDict[projectName] = [percentage, linesMatched]
	return
...
def dumpNumberOfLines(projectDict, fileName):
	...
	for projectName, stats in projectDict.items():
		percentage = stats[0]
		matchedLines = stats[1]
		if percentage == matchedLines == 0:
			csvwriter.writerow([projectName, 0])
		else:
			if percentage == 0:
				percentage = 0.5
			total_number_of_lines = math.ceil(matchedLines*100/percentage)
			csvwriter.writerow([projectName, total_number_of_lines])
	number_of_lines_file.close()

logFile = open('metrics-calculation-{0}.log'.format(datetime.datetime.now()), 'w')
sys.stdout = logFile
